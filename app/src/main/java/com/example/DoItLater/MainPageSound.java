...
public class MainPageSound extends AppCompatActivity {

    public static AlarmManager alarmpSound;  //!!!!
    public static Boolean checkSilenziosoo; //!!!!
    public static Boolean checkVibrazionee;
    public static Boolean checkSuoneriaa;
    private FloatingActionButton add;
    private Dialog dialog;
    private AppDatabase appDatabase;
    private RecyclerView recyclerView;
    private AdapterSound adapter;
    private List<Sounds> temp;
    private TextView empty;
    private PendingIntent intent1;
    private AlarmManager alarmManager;
    private RadioButton Silenzioso;
    private RadioButton Vibrazione;
    private RadioButton Suoneria;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        appDatabase = AppDatabase.geAppdatabase(MainPageSound.this);
        ...
        LinearLayoutManager linearLayoutManager = new LinearLayoutManager(MainPageSound.this);
        ...
        ItemTouchHelper.SimpleCallback simpleCallback = new ItemTouchHelper.SimpleCallback(0, ItemTouchHelper.LEFT | ItemTouchHelper.RIGHT) {
            ...
            public void onSwiped(@NonNull RecyclerView.ViewHolder viewHolder, int direction) {
                ...
                Sounds EventRemove = temp.get(position);
                Intent intent = new Intent(MainPageSound.this, NotifierAlarmSound.class);
                intent.putExtra("Silenzioso", EventRemove.getCheckSilenzioso());
                intent.putExtra("Vibrazione", EventRemove.getCheckVibrazione());
                intent.putExtra("Suoneria", EventRemove.getCheckSuoneria());
                ...
                intent1 = PendingIntent.getBroadcast(MainPageSound.this, EventRemove.getId(), intent, PendingIntent.FLAG_UPDATE_CURRENT);
    ...
    protected void onStart() {
        super.onStart();
        Intent intent4 = new Intent(android.provider.Settings.ACTION_NOTIFICATION_POLICY_ACCESS_SETTINGS);
        if(ActionsActivity.checkPermissionSound == false) {
            startActivity(intent4);
            Toast.makeText(MainPageSound.this, "Dai autotizzazione", Toast.LENGTH_SHORT).show();
            ActionsActivity.checkPermissionSound = true;
            finish();
        }
    }
    ...
    public void addReminder() {
        ...
        dialog = new Dialog(MainPageSound.this);
        dialog.setContentView(R.layout.floating_popup_sound);
        ...
        Silenzioso = dialog.findViewById(R.id.radioButtonSilenzioso);
        Vibrazione = dialog.findViewById(R.id.radioButtonVibrazione);
        Suoneria = dialog.findViewById(R.id.radioButtonSuoneria);
        ...
        selectDate.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                DatePickerDialog dialog = new DatePickerDialog(MainPageSound.this, new DatePickerDialog.OnDateSetListener() {
                    @Override
                    public void onDateSet(DatePicker view, final int year, final int month, final int dayOfMonth) {

                        newDate = Calendar.getInstance();
                        Calendar newTime = Calendar.getInstance();
                        TimePickerDialog time = new TimePickerDialog(MainPageSound.this, new TimePickerDialog.OnTimeSetListener() {
                            @Override
                            public void onTimeSet(TimePicker view, int hourOfDay, int minute) {
                                ...
                                if(newDate.getTimeInMillis()-tem.getTimeInMillis()>0)
                                    ...
                                    Toast.makeText(MainPageSound.this,"Invalid time",Toast.LENGTH_SHORT).show();
        ...
        add.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                checkSilenziosoo = Silenzioso.isChecked();
                sounds.setCheckSilenzioso(Silenzioso.isChecked());
                checkVibrazionee = Vibrazione.isChecked();
                sounds.setCheckVibrazione(Vibrazione.isChecked());
                checkSuoneriaa = Suoneria.isChecked();
                sounds.setCheckSuoneria(Suoneria.isChecked());
                Date remind = null;
                if(checkTime.isChecked() == true) {
                    remind = new Date(textDate.getText().toString().trim());
                    sounds.setRemindDate(remind);
                }
                else
                {
                ...
                sounds.setLatitudine(latitudine.getText().toString().trim());
                sounds.setLongitudine(longitudine.getText().toString().trim());
                sounds.setRaggio(raggio.getText().toString().trim());
                sounds.setCheckGeo(checkGeo.isChecked());
                sounds.setCheckTime(checkTime.isChecked());
                roomDAO.Insert(sounds);
                List<Sounds> l = roomDAO.getAllSounds();
                sounds = l.get(l.size() - 1);
                Log.e("ID chahiye", sounds.getId() + "");
                ...
                if (permission != PERMISSION_GRANTED || permission1 != PERMISSION_GRANTED) {
                    ActivityCompat.requestPermissions(MainPageSound.this, new String[]{ACCESS_FINE_LOCATION}, 1);
                    ActivityCompat.requestPermissions(MainPageSound.this, new String[]{ACCESS_COARSE_LOCATION}, 1);
                }
                ...
                if (entrata.isChecked() == true) {
                    gf = new Geofence.Builder()
                            .setRequestId(String.valueOf(sounds.getId()))
                            .setCircularRegion(Double.parseDouble(sounds.getLatitudine()), Double.parseDouble(sounds.getLongitudine()), Float.parseFloat(sounds.getRaggio()))
                            //.setRequestId("gf1")
                ...
                {
                    Intent intent = new Intent(v.getContext(), NotifierAlarmSound.class);
                    intent.putExtra("Silenzioso", checkSilenziosoo);
                    intent.putExtra("Vibrazione", checkVibrazionee);
                    intent.putExtra("Suoneria", checkSuoneriaa);
                    intent1 = PendingIntent.getBroadcast(v.getContext(), sounds.getId(), intent, PendingIntent.FLAG_UPDATE_CURRENT);

                    gfc.addGeofences(gfr, intent1)
                            .addOnSuccessListener(new OnSuccessListener<Void>() {
                                @Override
                                public void onSuccess(Void unused) {
                                    Toast.makeText(MainPageSound.this ,"GEO INSERITO", Toast.LENGTH_SHORT).show();
                                }
                            });
                }
                else if((checkGeo.isChecked() == false) && (checkTime.isChecked() == true))
                {
                    ...
                    intent.putExtra("Vibrazione", checkVibrazionee);
                    intent.putExtra("Suoneria", checkSuoneriaa);
                    intent.putExtra("RemindDate",sounds.getRemindDate().toString());
                    intent.putExtra("id",sounds.getId());
                    ...
                    intent1 = PendingIntent.getBroadcast(MainPageSound.this,sounds.getId(),intent,PendingIntent.FLAG_UPDATE_CURRENT);
                    ...
                    Toast.makeText(MainPageSound.this,"Inserted Successfully",Toast.LENGTH_SHORT).show();
                }
                else if((checkGeo.isChecked() == true) && (checkTime.isChecked() == true ))
                {
                    Intent intent = new Intent(MainPageSound.this,NotifierGeoSound.class);
                    intent.putExtra("Silenzioso", checkSilenziosoo);
                    intent.putExtra("Vibrazione", checkVibrazionee);
                    intent.putExtra("Suoneria", checkSuoneriaa);
                    intent.putExtra("RemindDate",sounds.getRemindDate().toString());
                    ...
                    intent.putExtra("checkRipetizioneSettimana", checkOgniSettimana.isChecked());
                    intent1 = PendingIntent.getBroadcast(MainPageSound.this,sounds.getId(),intent,PendingIntent.FLAG_UPDATE_CURRENT);
                    alarmpSound = (AlarmManager)getSystemService(ALARM_SERVICE);
    ...
    public void setItemsInRecyclerView(){
        ...
        temp = dao.orderThetableSound();
        if(temp.size()>0) {
            empty.setVisibility(View.INVISIBLE);
            recyclerView.setVisibility(View.VISIBLE);
        }
        adapter = new AdapterSound(temp);
